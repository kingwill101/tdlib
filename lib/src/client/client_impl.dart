...
import 'package:tdlib2/src/client/platform/platform.dart';
...
final class ClientImpl implements Client {
  ...
  Future<T> send<T extends TdObject>(TdFunction function) async {
    ...
    final jsonAsMap = function.toJson();
    final extra = ++_extraCount;
  ...
  T execute<T extends TdObject>(TdFunction function) {
    ...
    final result = _platform!.execute(function: function.toJson());
    final object = result.toTdObject();
    if (object == null) {
  ...
  Future<void> initialize({
    Iterable<TdFunction> beforeReceive = const [],
  }) async {
    assert(_state == _ClientState.created);
    await _platform!.initialize(
      beforeReceive: beforeReceive.map((function) => function.toJson()),
    );
  ...
  @override
  void destroy() {
    assert(_state != _ClientState.destroyed);
...
enum _ClientState { created, initialized, destroyed }
