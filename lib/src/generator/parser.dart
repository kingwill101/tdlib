...
class Parser {
  Parser({this.tlPath = 'tool/schema/td_api.tl'}) : source = null;

  final String tlPath;
  final String? source;

  Parser.fromSource(this.source) : tlPath = '';
  ...
  List<Class> parse() {
    ...
    for (final String line in lines) {
      ...
      if (fieldsRegx.hasMatch(line)) {
        ...
        // Skip type system declarations (primitive aliases, special syntax)
        final trimmedArgs = classArgs?.trim();
        if (trimmedArgs == '?' ||
            className == 'boolFalse' ||
            className == 'boolTrue' ||
            (trimmedArgs != null && trimmedArgs.contains('{t:Type}')) ||
            ['int32', 'int53', 'int64', 'bytes'].contains(className)) {
          continue;
        }

        final String parent = resolveParentType(group, className, returnType);
        final bool hasArgs = classArgs != null && classArgs.trim().isNotEmpty;
        final Map<String, String> args = hasArgs
            ? Map<String, String>.fromEntries(
                classArgs.trim().split(' ').where((arg) => arg.isNotEmpty).map((
                  String arg,
                ) {
                  final parts = arg.split(':');
                  if (parts.length != 2) {
                    throw FormatException(
                      'Invalid field format at line: $line',
                    );
                  }
